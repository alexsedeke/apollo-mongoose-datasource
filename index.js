...
class MongooseDataSource extends DataSource {
   ...
   * @param {object} [{}] optional - Optional. Specifies the fields to return in the documents
   ...
   */
  async all(options = {}) {
    const filter = util.transform.toMongooseFilterExpression(options.filter);
  ...
  async list(options = {}) {
    ...
    const datafilter = util.transform.toMongooseFilterExpression(filter);
   ...
   * @returns {object|error} Retuns created document object or return database error.
   */
  async add(document) {
   ...
   * @returns {object|error} Retuns removed document object or return database error.
   */
  async delete(id) {
    try {
  ...
  /**
   * Remove multiple document by it's id.
   * @param {string\array} id Document id as string or array of id`s.
   * @returns {object|error} Retuns removed document object or return database error.
   */
  ...
  /**
   * Update multiple document by list of id's.
   * @param {*} id Document id
   * @param {*} documentUpdate Document object with updated information
   */
  async updateMany(id, documentUpdate) {
    ...
    try {
      ...
      const cleanedDocumentUpdate = util.cleanDocumentUpdate(documentUpdate);
